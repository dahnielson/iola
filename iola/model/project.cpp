...
public:
	...
	{
		m_pkSequenceStart.set_timecode(timecode);
	}
	...
	void program_export_edl(boost::filesystem::path edl)
	{
		...
		for (int i = 0; i < number_of_clips; ++i)
		{
			...
			iola::model::timecode pkInTC(m_iTimebase, pkInfo->frame_in);
			iola::model::timecode pkOutTC(m_iTimebase, pkInfo->frame_out);
			iola::model::timecode pkStartTC(m_iTimebase, pkInfo->start);
			iola::model::timecode pkEndTC(m_iTimebase,pkInfo->start +  pkInfo->frame_out - pkInfo->frame_in);
...
private:
	...
	iola::model::timecode m_pkSequenceStart;

	Mlt::Event* m_pkSourceProducerChangeEvent;
	Mlt::Event* m_pkProgramProducerChangeEvent;

	static void source_producer_change_callback(mlt_producer, project_implementation* self)
	{
		self->on_source_producer_change_signal();
	}

	static void program_producer_change_callback(mlt_producer, project_implementation* self)
	{
...
{
	return new project_implementation();
}
