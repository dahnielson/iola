...
clip_element::clip_element(const std::string strName) :
        m_strName(strName),
	m_pkName(0),
	m_pkDuration(0),
	m_pkFile(0),
...
void
clip_element::attribute(std::string strKey, std::string strValue)
{
	if (strKey == "id")
		m_strID = strValue;
}
...
void
clip_element::text(std::string strText)
{
}

void
clip_element::xml(std::ostream& osXML)
{
	if (m_strID.empty())
